...
class App {
    ...
    /* initLeftSidebar() {
        ...
        const scrollActiveItemToCenter = () => {
            const activeItem =
                bestMatch || sideNav.querySelector("a.side-nav-link.active");
            ...
            if (activeItem && scrollContainer) {
                ...
                // Keep active item around the center of the visible sidebar area
                const targetTop =
                    scrollContainer.scrollTop +
                    (itemRect.top - containerRect.top) -
                    (scrollContainer.clientHeight / 2 - itemRect.height / 2);

                scrollToPosition(scrollContainer, Math.max(targetTop, 0), 600);
            }
        };

        // Wait for collapse transition to finish, then scroll once
        setTimeout(scrollActiveItemToCenter, 420);
    ...
    initLeftSidebar() {
        ...
        // Match current URL
        // We strip query params and hash for cleaner matching
        const currentUrl = window.location.href.split(/[?#]/)[0];
        const allLinks = sideNav.querySelectorAll("a");
        ...
        allLinks.forEach((link) => {
            // Ignore hash/js links or empty hrefs
            ...
            // Get absolute URL for the link and strip params/hash
            const linkUrl = new URL(
                link.href,
                window.location.origin,
            ).href.split(/[?#]/)[0];

            // 1. Data Attribute Match (Override)
            // Allows manual override via data-active-match="part/of/url"
            const activeMatch = link.getAttribute("data-active-match");
            if (activeMatch && currentUrl.includes(activeMatch)) {
                // Give extremely high priority to explicit overrides
                if (activeMatch.length + 10000 > maxMatchLength) {
                    bestMatch = link;
                    maxMatchLength = activeMatch.length + 10000;
                }
                return;
            }

            // 2. Longest Prefix Match (Wildcard behavior)
            // Check if Current URL starts with this Link URL
            // This mimics `routeIs('prefix*')` behavior.
            // Example:
            // Current: .../icons/tabler-full
            // Link:    .../icons/tabler
            // Result:  Match!
            if (currentUrl.startsWith(linkUrl)) {
                // We pick the matching link that is the LONGEST.
                // This ensures /icons/tabler (longer) beats /icons (shorter)
                if (linkUrl.length > maxMatchLength) {
                    bestMatch = link;
        ...
        if (bestMatch) {
            ...
            sideNav.querySelectorAll("a.active, li.active").forEach((el) => {
                el.classList.remove("active");
            });
            ...
            while (currentElement && currentElement !== sideNav) {
                ...
                const parentCollapse = currentElement.closest(".collapse");
                if (parentCollapse) {
                    // Use Bootstrap API to show. toggle: false prevents toggling if already shown
                    const bsCollapse = new bootstrap.Collapse(parentCollapse, {
                        toggle: false,
                    });
                    bsCollapse.show();

                    parentCollapse.classList.add("show"); // Ensure class is present immediately

                    // Also mark the <li> that contains the collapse as active (parent menu item)
                    const collapseParentLi = parentCollapse.closest("li");
                    if (collapseParentLi) {
                        collapseParentLi.classList.add("active");
                    }
...
class LayoutCustomizer {
    ...
    changeTheme(color) {
        ...
        this.html.setAttribute(
            "data-bs-theme",
            color === "system" ? this.getSystemTheme() : color,
        );
        this.setSwitchFromConfig();
    }
...
class I18nManager {
    constructor({
        ...
        this.selectedLanguage = document.documentElement.lang || defaultLang;
    ...
    setLanguage(lang) {
        ...
        // The language switch is now handled by the server (cookie + reload)
        // so we don't need to manually set localStorage here for persistence.
        ...
        if (this.element) {
            ...
            if (data) {
                const parsed = data
                    ...
                    .map((c) => c.trim())
                    .map((c) =>
                        c.startsWith("#") || c.includes("rgb") ? c : ins(c),
                    );
    ...
    static reSizeAll() {
        if (CustomEChart.instances.length > 0) {
            for (const instance of CustomEChart.instances) {
                if (
                    instance.element &&
                    instance.element.offsetParent !== null
                ) {
