...
const githubdata = {
    series: Array.from({ length: 365 }, (_, index) => {
        ...
        const trend = 90 + (index * 0.12);
        const wave = Math.sin(index / 9) * 14;
        return [base + (index * 86400000), Math.round(trend + wave)];
    })
};

const buildSeries = (length, start, phase = 0) =>
    Array.from({ length }, (_, i) => ({
        value: Math.round(start + Math.sin((i + phase) / 2.2) * 1800000 + (i * 42000))
    }));

const dataSeries = [
    buildSeries(12, 11000000, 0),
    buildSeries(18, 9000000, 1.6),
    buildSeries(12, 7000000, 2.8)
];
//
// Basic Area Chart
//
