...
const generateOhlcData = (count = 120, startDate = new Date('2017-01-01').getTime(), base = 6600) => {
    let lastClose = base;
    const candles = [];
    const volumes = [];
    ...
    for (let i = 0; i < count; i++) {
        ...
        const open = Number((lastClose + (Math.sin(i / 7) * 18)).toFixed(2));
        const close = Number((open + (Math.cos(i / 5) * 22)).toFixed(2));
        const high = Number((Math.max(open, close) + 8 + (i % 5)).toFixed(2));
        const low = Number((Math.min(open, close) - 8 - (i % 4)).toFixed(2));
...
const { candles: seriesData, volumes: seriesDataLinear } = generateOhlcData();
//
...
// Combo Candlestick
//
const chartCandlestick = new CustomApexChart({
        ...
        chart: {
            ...
            events: {
                selection: function (chart, e) {
                    chartCandlestick.chart?.updateOptions({
                        xaxis: {
                            min: e.xaxis.min,
                            max: e.xaxis.max
                        }
        ...
        xaxis: {
            ...
            labels: {
                formatter: function (val) {
                    return window.moment(val).format('MMM DD HH:mm')
                }
            }
        },
        yaxis: {
